...
__device__ void load_data_to_shared_memory(cuDoubleComplex const* A, size_t lda,
    cuDoubleComplex const* B, size_t ldb,
    cuDoubleComplex A_thread_block_tile[BLOCK_TILE_SIZE_Y][BLOCK_TILE_SIZE_K + BLOCK_TILE_SKEW_SIZE_K],
    cuDoubleComplex B_thread_block_tile[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_X + BLOCK_TILE_SKEW_SIZE_X],
...
__global__ void gemm_v04(size_t m, size_t n, size_t k, GpuMatrix A, size_t lda, GpuMatrix B, size_t ldb, GpuMatrix C, size_t ldc) {
    ...
    __shared__ cuDoubleComplex A_thread_block_tile[BLOCK_TILE_SIZE_Y][BLOCK_TILE_SIZE_K];
    __shared__ cuDoubleComplex B_thread_block_tile[BLOCK_TILE_SIZE_K][BLOCK_TILE_SIZE_X];
    ...
    cuDoubleComplex C_thread_results[THREAD_TILE_SIZE_Y][THREAD_TILE_SIZE_X] = { make_cuDoubleComplex(0, 0) };
    // A_vals is cached in the register.
    cuDoubleComplex A_vals[THREAD_TILE_SIZE_Y] = { make_cuDoubleComplex(0, 0) };
    // B_vals is cached in the register.
    cuDoubleComplex B_vals[THREAD_TILE_SIZE_X] = { make_cuDoubleComplex(0, 0) };
...
//__global__
//void matMultKernel(cuDoubleComplex* A, int Am, int An, cuDoubleComplex* B, int Bm, int Bn, cuDoubleComplex* res) {
//
...
__global__ void MatMulKernel(GpuMatrix A, GpuMatrix B, GpuMatrix C)
{
    ...
    for (int m = 0; m < (A.width / BLOCK_SIZE); ++m) {
        ...
        __shared__ cuDoubleComplex As[BLOCK_SIZE][BLOCK_SIZE];
        __shared__ cuDoubleComplex Bs[BLOCK_SIZE][BLOCK_SIZE];
        ...
        for (int e = 0; e < BLOCK_SIZE; ++e)
            Cvalue = cuCadd(Cvalue, cuCmul(As[row][e], Bs[e][col]));
...
__device__ void loadFromGmem(int N, int K, const cuDoubleComplex* A, const cuDoubleComplex* B,
    cuDoubleComplex* As, cuDoubleComplex* Bs, int innerRowA, int innerColA,
...
__device__ void
processFromSmem(cuDoubleComplex* regM, cuDoubleComplex* regN, cuDoubleComplex* threadResults, const cuDoubleComplex* As,
    const cuDoubleComplex* Bs, const uint warpRow, const uint warpCol,
                ...
                for (uint resIdxM = 0; resIdxM < TM; ++resIdxM) {
                    for (uint resIdxN = 0; resIdxN < TN; ++resIdxN) {
                        threadResults[(wSubRowIdx * TM + resIdxM) * (WNITER * TN) + (wSubColIdx * TN) + resIdxN] =
                            cuCadd(threadResults[(wSubRowIdx * TM + resIdxM) * (WNITER * TN) + (wSubColIdx * TN) + resIdxN], cuCmul(regM[wSubRowIdx * TM + resIdxM], regN[wSubColIdx * TN + resIdxN]));
                    }
                }
            }
        }
...
__global__ void __launch_bounds__(NUM_THREADS) sgemmWarptiling(int M, int N, int K, cuDoubleComplex* A, cuDoubleComplex* B, cuDoubleComplex* C) {
    ...
    cuDoubleComplex threadResults[WMITER * TM * WNITER * TN] = { make_cuDoubleComplex(0, 0) };
    // we cache into registers on the warptile level
    cuDoubleComplex regM[WMITER * TM] = { make_cuDoubleComplex(0, 0) };
    cuDoubleComplex regN[WNITER * TN] = { make_cuDoubleComplex(0, 0) };
    ...
    for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
        for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
            ...
            cuDoubleComplex* C_interim = C + (wSubRowIdx * WSUBM) * N + wSubColIdx * WSUBN;
            for (uint resIdxM = 0; resIdxM < TM; resIdxM += 1) {
                for (uint resIdxN = 0; resIdxN < TN; resIdxN += 4) {
                    ...
                    tmp.x = cuCadd(threadResults[i + 0], tmp.x);
                    tmp.y = cuCadd(threadResults[i + 1], tmp.y);
                    tmp.z = cuCadd(threadResults[i + 2], tmp.z);
                    tmp.w = cuCadd(threadResults[i + 3], tmp.w);
                    // write back
                    reinterpret_cast<cuDoubleComplex4*>(
                        &C_interim[(threadRowInWarp * TM + resIdxM) * N +
...
void Matrix::gpuMultIn(Matrix& A, Matrix& B, Matrix& res) {
    ...
    cuDoubleComplex* tempA = (cuDoubleComplex*)malloc(size);

    for (int i = 0; i < A.m; ++i) {
        for (int j = 0; j < A.n; ++j) {
            tempA[j + i * A.n] = complexToCudaComplex(A.entry(i, j));
        }
    }
    ...
    cuDoubleComplex* tempB = (cuDoubleComplex*)malloc(size);

    for (int i = 0; i < B.m; ++i) {
        for (int j = 0; j < B.n; ++j) {
            tempB[j + i * B.n] = complexToCudaComplex(B.entry(i, j));
        }
    }
    ...
    cuDoubleComplex* tempRes = (cuDoubleComplex*)malloc(size);

    cudaMemcpy(tempRes, dev_res.elements, size, cudaMemcpyDeviceToHost);

    for (int i = 0; i < A.m; ++i) {
        for (int j = 0; j < B.n; ++j) {
            res.entry(i, j) = cudaComplexToComplex(tempRes[j + i * B.n]);
        }
    }
